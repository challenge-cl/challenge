...
class ProxyHandler:
    ...
    def __init__(self):
        ...
        self.auth_token_generator = AuthToken(KEY)
    ...
    async def status(self, request):
        ...
        data = {'startupTime': server_startup_time, 'totalProxyedRequests': self.proxyed_request_counter}
    ...
    async def post_proxy(self, request):
        ...
        request_headers[self.AUTH_TOKEN_HEADER_NAME] = self.auth_token_generator.generate(auth_payload)
        async with aiohttp.ClientSession() as session:
            async with session.post(request.path_qs, data=body) as resp:
...
def main():
    ...
    app.add_routes([web.get('/metrics', MetricsView),
                    web.get('/status', proxy_handler.status),
                    web.post('/{path:.*?}', proxy_handler.post_proxy)])
